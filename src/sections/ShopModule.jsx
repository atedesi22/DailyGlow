      ...
      <div className="flex justify-center gap-10 mb-10">
        {['Homme', 'Femme'].map(g => (
          <button 
            ...
            className={`text-xl font-serif pb-2 ${activeGender === g ? 'text-pink-900 border-b-2 border-pink-900' : 'text-gray-400'}`}
          >
            {g}
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 md:gap-8">
        {PRODUCTS.filter(p => p.gender === activeGender).map(product => (
          <div key={product.id} className="bg-white p-3 rounded-2xl shadow-sm border border-pink-50">
            ...
            <h3 className="font-serif text-sm text-pink-950">{product.name}</h3>
            <p className="text-pink-700 font-bold text-sm mb-3">{product.price} FCFA</p>
            <button 
              onClick={() => onAddToCart(product)}
              className="w-full py-2 bg-pink-950 text-white rounded-lg text-xs flex items-center justify-center gap-2"
            >
